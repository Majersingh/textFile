...
# 1. Define the schema (What the LLM must extract)
class TicketSchema(BaseModel):
    origin: str = Field(description="The departure city or airport (e.g., Delhi)")
    destination: str = Field(description="The arrival city or airport (e.g., Mumbai)")
    travel_date: str = Field(description="The date of travel (e.g., 1st March)")

# 2. Define the tool using the schema
@tool(args_schema=TicketSchema)
def book_ticket_tool(origin: str, destination: str, travel_date: str) -> str:
    """
    Use this tool when a user wants to book a flight or train ticket.
    It requires the starting point, destination, and date.
    """
    # This is where your actual booking logic (API calls) would go
    print(f"Searching for tickets from {origin} to {destination} on {travel_date}...????????????????????????")
    return f"Searching for tickets from {origin} to {destination} on {travel_date}..."
...
react_agent = create_react_agent(
    llm,
    tools=[book_ticket_tool, calculator, web_search],
)
...
"""
You are a helpful Document Analyzer Assistant. Your entire knowledge base for this conversation is limited strictly to the text provided in the "Context" section below.
Core Directives:
When the user says "this," "it," "the document," or "that," they are referring specifically to the text provided in the Context.
If the user asks "What is this about?" or "Summarize it," provide an overview based only on the Context.
Do NOT use outside knowledge or your internal training data.
If the answer cannot be found in the Context, reply exactly: "I don't know based on the document."
Context:
{context}
User Question:
{question}
